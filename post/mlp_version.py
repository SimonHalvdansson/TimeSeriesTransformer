...
def plot_dataset(ds):
    plt.figure(figsize=(16, 5), dpi=300)  # Set high resolution with dpi=300
    
    x = np.arange(context_len + output_len)
    
    plt.plot(x[:context_len], ds[0][0].numpy(), color='blue', label='Input')
    plt.plot(x[context_len:], ds[0][1].numpy(), color='orange', label='Target')
    
    plt.xlabel("Reading", fontsize=12)  # Ensure text size remains reasonable
    plt.ylabel("Temperature (C)", fontsize=12)
    plt.title('Weather dataset example', fontsize=14)
    plt.legend(fontsize=12)
    
    plt.tight_layout()  # Adjust layout to prevent text clipping
    plt.savefig("dataset_example.png", format="png", dpi=300)  # Save the figure as a high-res PNG
    plt.show()
