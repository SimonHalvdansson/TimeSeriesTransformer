...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = self.points[
            idx + self.context_len : idx + self.context_len + self.output_len, channel
        ]
...
class TimeSeriesTransformer(nn.Module):
    def __init__(
        ...
        if context_len % big_patch_len != 0:
            raise Exception(
                "context_len needs to be a multiple of patch_len*big_patch_multiple"
            )
        ...
        self.patch_encoder = ResidualBlock(
            input_dim=patch_len * channels,
            output_dim=d_model,
            hidden_dim=d_model,
            dropout=dropout,
            apply_ln=True,
        )
        self.big_patch_encoder = ResidualBlock(
            input_dim=big_patch_len * channels,
            output_dim=d_model,
            hidden_dim=d_model,
            dropout=dropout,
            apply_ln=False,
        ...
        self.pos_embedding = self.sinusoidal_positional_embedding(
            self.patches + self.big_patches + 1, d_model
        ).to(device)
    ...
    def encode_patches(self, x, patches, patch_len, encoder):
        x = x.view(x.shape[0], patches, patch_len, self.channels)
        encoded_patches = []
    ...
    def forward(self, x):
        ...
        if x.shape[2] != self.channels:
            raise Exception("input_channels dimension mismatch")
        ...
        encoded_big_patches = self.encode_patches(
            x, self.big_patches, self.big_patch_len, self.big_patch_encoder
        )
...
if __name__ == "__main__":
    ...
    traffic_ds = GenericCSVDataset(
        path="data/traffic/traffic.csv", valid_channels=list(range(1, 862))
    )

    ds_train = TimeSeriesDataset(
        traffic_ds, context_len, output_patch_len, split="train"
    )
